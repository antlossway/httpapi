...
cur.execute("select api_key,api_secret, b.id as billing_id, b.company_name, webuser_id, w.username as webuser_name,product_id, p.name as product_name from api_credential a join webuser w on w.id=a.webuser_id join billing_account b on w.billing_id = b.id join product p on p.id=a.product_id;")
rows = cur.fetchall()
for row in rows:
    (api_key,api_secret,billing_id,company_name,webuser_id,webuser_name,product_id,product_name) = row
    ac = {
        "api_key": api_key,
